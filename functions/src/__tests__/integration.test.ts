...
test("Stripe provider failure leaves a failed checkout record and never returns success", async () => {
...
test("Stripe webhook duplicate delivery is idempotent and out-of-order lifecycle events do not duplicate welcome effects", async () => {
...
test("tampered enrollment agreement payload is rejected before any durable artifact is written", async () => {
  const identity = await createTestIdentity();
...
test("an authorized agreement is persisted in Firestore and Storage, while a different family is refused", async () => {
  const owner = await createTestIdentity();
  const other = await createTestIdentity();
...
test("booking create and cancellation handlers persist their real side effects through provider transports", async () => {
...
test("a retried welcome invocation sends once and records its outbox progress", async () => {
  const identity = await createTestIdentity();
  await seedSyntheticDocument(`users/${identity.uid}`, {
    email: identity.email,
    displayName: "Synthetic Parent",
...
test("unsubscribe withdrawal changes the real user record and is idempotent", async () => {
  const identity = await createTestIdentity();
  await seedSyntheticDocument(`users/${identity.uid}`, {
    email: identity.email,
    unsubscribed: false,
