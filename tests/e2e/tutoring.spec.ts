...
test.describe('/tutoring marketing page (Epic H1)', () => {
  ...
  test('renders all 3 PricingCard prices and the IEP review price', async ({ page }) => {
    ...
    // Drop-in
    await expect(
      page.getByTestId('pricing-price-tutoring-drop-in')
    ).toContainText('$125')
    // Weekly (featured)
    await expect(
      page.getByTestId('pricing-price-tutoring-weekly')
    ).toContainText('$460')
    // Twice-weekly
    await expect(
      page.getByTestId('pricing-price-tutoring-twice-weekly')
    ).toContainText('$880')
    // IEP review banner
    await expect(page.getByTestId('pricing-price-iep-review')).toContainText(
      '$250'
    )
  })
  ...
  test('pricing CTAs link to the correct Stripe endpoints', async ({ page }) => {
    ...
    const dropInCta = page
      .getByRole('link', { name: /Book a single session/i })
      .first()
    ...
    const twiceCta = page.getByRole('link', {
      name: /Start twice-weekly subscription/i,
    })
