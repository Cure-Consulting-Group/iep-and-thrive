...
1. Implement a local numeric challenge that regenerates a new problem on every incorrect answer,
   with no network call, no stored answer or raw response, and no persisted attempt counter.
   Rate-limit with a short increasing delay rather than a lockout: a parent who is slow at mental
   arithmetic must never be locked out of their own app, and dyscalculia co-occurs with dyslexia
   in this population.
...
- **Given** a child reaches a gated adult action, **when** an incorrect answer is entered, **then**
  the action remains locked, a new problem is generated, no attempt counter is persisted, and no
  response is uploaded. There is no lockout state and no path by which an adult is permanently
  refused.
...
Run reducer tests for correct, incorrect, repeated-incorrect, cancel, and relaunch paths, asserting
that no lockout state survives a relaunch. Perform VoiceOver
